...
function getData() {
    ...
    xhr.onload=function(){
        if(xhr.status==200){
            ...
            Wrap.addEventListener('mousemove', updateList,{once:true});//更新下拉選單
        }
    }  
}
...
function showtime(e) {
    ...
    var targetinnerhtml = e.target.value||e.target.innerText;//熱門區選取到的HTML內容 EX:三民區
    var str = '';//過濾後資料字串
    var strnum = '';//資料頁區字串
    ...
    e.preventDefault();
    console.log(e);
    filterData=data.result.records.filter(function(el,i){
        return (el.Zone===targetValue||el.Zone===targetinnerhtml);
    });
    filterData.forEach(function(el,i){
        if(i<4){
           str+='<div class="card"><div class="card-header" style="background-image: url(' + el.Picture1 + ')"><h4>' + el.Name + '</h4><p>' + el.Zone + '</p><div class="clear-fix"></div></div><div class="card-p"><div class="time-icon"></div><p>' + el.Opentime + '</p></div><div class="card-p"><div class="position-icon"></div><p>' + el.Add + '</p></div><div class="card-p"><div class="phone-icon"></div><p>' + el.Tel + '</p></div></div>';
        }
        num=num+1
    });
    content.innerHTML = '<h3>' + h3 + '</h3><div class="card-group" id="card-Group">' + str + '</div>';
    option.value = targetinnerhtml;
